...
Serialize concurrent MLX GPU inference calls so that multiple RAG backends or concurrent requests do not contend on the shared Metal GPU, preventing GPU thrashing and reducing wall-clock time when multiple backends are dispatched simultaneously.
