...
Wire the `APIDocRAG` DSPy module into `ApiDocPipelineManager.query()` as the primary answer-generation path, with circuit-breaker fallback to prompt-based generation, output mapping to `ApiDocQueryResponse`, and a configuration toggle.
