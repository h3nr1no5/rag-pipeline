# DOCX Heading Level Detection

## Purpose

The DOCX parser detects heading levels by matching paragraph style names against known heading prefixes (e.g., `"Heading"`, `"Title"`, `"Caption"`). This capability maps DOCX paragraph style names to numeric heading levels used by the heading stack for table-to-interface assignment. It covers numbered heading styles (`"Heading 1"`–`"Heading 6"`), the bare `"Heading"` style (Word's default level-1 heading), and special styles like `"Title"` and `"Subtitle"`.
