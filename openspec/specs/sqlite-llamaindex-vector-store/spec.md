...
Replace the Chroma-backed vector store with a SQLite-native approach for the LlamaIndex RAG backend. Chunks are loaded from SQLite as `NodeWithScore` objects, dense retrieval uses exact dot-product on SQLite-stored embeddings, and all Chroma dependencies are removed.
