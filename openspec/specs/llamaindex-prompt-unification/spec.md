...
Standardize the LlamaIndex RAG backend to use the shared prompt builder (`build_prompt()` from `prompt_builder.py`) across both streaming and non-streaming paths, eliminating the divergent inline prompt format that caused silent answer failures.
