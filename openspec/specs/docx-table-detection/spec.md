...
Detect the type of each DOCX table by examining the nearest preceding bold paragraph label (Functions, Properties, Enumerated types, Error codes, Records / structures), inherit types across consecutive tables, and fall back to keyword matching when no label matches.
