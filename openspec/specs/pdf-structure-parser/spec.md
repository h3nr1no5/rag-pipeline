...
Extract a structured, hierarchical representation of PDF documents — including text with positional data, headings at multiple levels, code blocks, tables, lists, and reading order — consumable by downstream enrichment and chunking stages. The parser handles multi-column layouts, layout analysis failures, and fallback scenarios with structured error reporting.
