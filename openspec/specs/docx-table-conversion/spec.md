...
Convert detected DOCX tables into structured API interface data by dispatching to type-specific positional extractors (method, property, enum, error_code, record) that interpret columns by position, and capture interface descriptions from heading paragraphs.
