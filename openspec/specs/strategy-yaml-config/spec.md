...
Define how system chunking strategies are configured via a YAML file (`config/strategies.yaml`) at application startup, replacing hardcoded strategy seeding with a declarative source of truth. Also defines the additive `config` JSON column on `ChunkingStrategy`, the `GET /strategies/types` endpoint for UI client form rendering, and the configurable `min_chunk_length` parameter.
  ...
  api-docs:
    ...
    config:
      ...
      heading_policy:
        ...
        section_levels: [3]
        ignore_levels: []
      # Note: method_table config has been removed. Column positions are
      # hardcoded per table type in the positional extractors.
      max_depth: 3
...
The `chunking_strategies` table SHALL gain a nullable `config` JSON column. This column SHALL store type-specific configuration parameters as a JSON object. For recursive and semantic strategies, `config` SHOULD be `null`. For api-docs strategies, `config` SHALL contain `type_patterns`, `heading_policy`, and formatting parameters. (The `method_table` config was previously part of this schema but has been removed — column positions are now hardcoded per table type in the extractors.)
