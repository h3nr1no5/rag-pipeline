...
Execute selected RAG backends sequentially in a specified order rather than concurrently. Each backend completes and stores its result before the next begins, with an overall timeout and frontend polling for incremental results.
