...
Load all models (cross-encoder reranker, LLM, embedder, DSPy LM) during FastAPI server startup via a lifespan background task, using module-level singletons as the single source of truth for model availability, and reporting status directly from singletons via the health endpoint.
