...
### 1. Reduce candidate pool: `internal_top_k` from 20 → 10
**Why**: Currently BM25 returns 40 docs (`internal_top_k * 2`) and FAISS returns 40, producing ~66 unique candidates. By reducing `internal_top_k` to 10, BM25 and FAISS each return 20 docs → ~30 unique candidates. This halves cross-encoder work (from ~307s to ~150-170s with 1-2 batches instead of 3).
...
### 2. Error key in frontend exception handlers
...
### 3. No increase to 180s frontend timeout
...
### 4. LangChain-specific timeout in backend
...
- **[Edge case] Query near 180s boundary**: Candidate pool reduction alone may not be enough for some queries; cross-encoder on ~30 docs still takes ~150-170s, leaving little margin. → Mitigation: Backend `asyncio.wait_for()` at 160s ensures clean failure, and improved error handling shows a clear "query timed out — try rephrasing" message. Rarely, users may need to retry on a less loaded server.
