...
- [ ] 4.1 Add `parent_interface: str | None = None` field to `APIEnum` in `src/domain/rag/api_docs/model/models.py`
- [ ] 4.2 Add `parent_interface: str | None = None` field to `APIErrorCode` in `src/domain/rag/api_docs/model/models.py`
- [ ] 4.3 Add new `APIRecordField` model to `src/domain/rag/api_docs/model/models.py`:
  ```python
  class APIRecordField(BaseModel):
      ...
      description: str = ""
  ```
- [ ] 4.4 Add new `APIRecord` model to `src/domain/rag/api_docs/model/models.py`:
  ```python
...
- [ ] 5.1 Add `get_config_value(strategy, key, default)` helper to safely read from `strategy.config` dict (handles null config)
- [ ] 5.2 Modify `_extract_interface_name()` in `src/domain/rag/api_docs/extraction/converter.py` to accept the strategy's `type_patterns` config and detect entity type:
...
- [ ] 5.3 Modify `_get_or_create_interface()` to accept a `heading_policy` config:
...
- [ ] 5.4 Update `process_document()` (main converter entry point) to pass strategy config through to `_get_or_create_interface()` and `_extract_interface_name()`
- [ ] 5.5 Update `_process_method_table()` to use `method_table.return_type_col` and `method_table.name_col` from config (with fallback to 0 and 1)
- [ ] 5.6 Populate `parent_interface` on enums and error codes during conversion:
...
- [ ] 5.7 Add `self.records: list[APIRecord] = []` to `DocumentConverter.__init__()` — new instance attribute for collected records
- [ ] 5.8 Add `elif table_type == "record":` branch in `convert()` method:
...
- [ ] 5.9 Implement `_convert_record_table(self, table: RawTable) -> APIRecord | None` method:
...
- [ ] 5.10 Update `convert()` return dict to include `"records"` key with `self.records`

## 6. Graph Builder — nested chunk nodes + entity metadata

- [ ] 6.1 Modify `ChunkGraphBuilder` in `src/domain/rag/api_docs/chunking/builder.py` to read `base_interface` from `APIInterface`:
...
- [ ] 6.2 Pass `max_depth` and `include_entities` config through to graph builder — skip entities not in whitelist, limit tree depth
- [ ] 6.3 Add `interface_name` to enum node metadata in `_add_enum()` — read from `enum_def.parent_interface` field (should match pattern used by method nodes)
- [ ] 6.4 Add `interface_name` to error-code node metadata in `_add_error_code()` — read from `ec.parent_interface` field
- [ ] 6.5 Verify that flat entities (enums/records/error_codes) remain at depth 0 regardless of heading position
- [ ] 6.6 Add `_add_record(self, graph, record: APIRecord, source_doc)` method to `ChunkGraphBuilder`:
...
- [ ] 6.7 Update `build()` method signature to accept `records: list[APIRecord] | None = None` and iterate over records with `_add_record()`

## 7. Formatter — formatting parameters

- [ ] 7.1 Modify `ChunkTextFormatter` in `src/domain/rag/api_docs/chunking/formatter.py` (or equivalent) to accept formatting params:
...
- [ ] 7.2 Pass formatting params from strategy config through the processing pipeline

## 8. Configurable min_chunk_length

- [ ] 8.1 Modify `src/domain/services/chunking.py` line 44 — replace hardcoded `20` with `self.strategy.config.get("min_chunk_length", 20)` when `config` is not None
- [ ] 8.2 Verify `min_chunk_length` is exposed in the GET /strategies/types schema for recursive and semantic types

## 9. Tests

- [ ] 9.1 Add unit test for YAML seeding: create temp YAML file, call seeder, verify strategies are created/upserted in DB
- [ ] 9.2 Add unit test for YAML fallback: verify hardcoded seeding still works when YAML file is missing
- [ ] 9.3 Add unit test for converter with `type_patterns`: headings matching patterns create correct entity types; non-matching headings become sections
- [ ] 9.4 Add unit test for converter with `heading_policy`: interface_levels, section_levels, ignore_levels produce correct nesting; enums/records stay flat
- [ ] 9.5 Add unit test for converter with `interface_levels` gaps: heading levels not contiguous produce correct depth assignment
- [ ] 9.6 Add unit test for converter populating `parent_interface` on enums: enum under IFooBar heading gets `parent_interface="IFooBar"`; enum before any heading gets `None`
- [ ] 9.7 Add unit test for converter populating `parent_interface` on error codes: same pattern as enum test
- [ ] 9.8 Add unit test for converter processing record tables:
...
- [ ] 9.9 Add unit test for graph builder nesting: interfaces with `base_interface` create parent-child chunk structure
- [ ] 9.10 Add unit test for graph builder metadata: enum nodes contain `interface_name` in metadata populated from `parent_interface`
- [ ] 9.11 Add unit test for graph builder metadata: record nodes contain `interface_name` in metadata populated from `record.parent_interface`
- [ ] 9.12 Add unit test for configurable `min_chunk_length`: recursive service rejects chunks below threshold from config
- [ ] 9.13 Add unit test for GET /strategies/types: returns correct schema shapes for all three engine types
- [ ] 9.14 Add unit test for formatter params: detailed vs compact output, signature/description toggling
- [ ] 9.15 Run test suite: `uv run pytest tests/ -v -x` — fix any failures

## 10. Documentation

- [ ] 10.1 Write `docs/chunking-strategies.md` with:
