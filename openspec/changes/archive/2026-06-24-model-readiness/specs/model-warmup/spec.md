...
**FROM**: The system begins loading cross-encoder reranker and LLM models during server startup.
**TO**: The system SHALL warm up all 4 models (cross_encoder, llm, embedder, dspy_lm) during server startup via `asyncio.create_task`. Loading SHALL be non-blocking (run in `asyncio.to_thread`) so the server accepts requests immediately.

The system SHALL track warmup status in a unified `WarmupState` singleton using a dictionary keyed by model name, where each entry is a `ModelStatus` with fields: `model` (name string), `status` (`queued`, `loading`, `ready`, `error`, `permanent_error`), `progress` (0-100 integer), `message` (status text like "Downloading..."), and `error` (error detail string).
...
**FROM**: 2 models (cross-encoder, llm) returned.
**TO**: All 4 models (cross_encoder, llm, embedder, dspy_lm) SHALL be returned with status, model name, progress, and optional error. Additionally, a `message` field SHALL provide a human-readable status description.
...
### Requirement: LangChain error check SHALL migrate to unified gate

**FROM**: LangChain routes have inline `warmup_state.cross_encoder.status == "error"` checks.
**TO**: The unified gate handles all cross-encoder checks. The inline check SHALL be removed.

This requirement is specified in detail in the `model-readiness-gate` spec.

## REMOVED Requirements

### Requirement: Score normalization SHALL be removed

**Reason**: This requirement was already implemented in a prior change. It is unrelated to the model readiness scope and is retained in the main spec for reference.
**Migration**: No action needed — the score normalization was already removed in a previous implementation.
