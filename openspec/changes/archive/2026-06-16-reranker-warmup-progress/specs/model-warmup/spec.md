...
- **THEN** the cross-encoder model (`CrossEncoder(gte-reranker-modernbert-base)`) SHALL be instantiated inside `asyncio.to_thread()`
...
- **WHEN** the server starts with models already loaded (e.g., no `--reload` reset)
...
The LangChain pipeline SHALL NOT apply min-max normalization to reranker scores. The raw [0,1] softmax scores from `gte-reranker-modernbert-base` SHALL be passed through as-is.

#### Scenario: Scores pass through without normalization
- **WHEN** the LangChain backend returns search results
- **THEN** the `score` field in each result SHALL be the raw output of the cross-encoder (range [0,1])
