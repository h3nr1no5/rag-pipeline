...
- **AND** the cache key SHALL incorporate the `link_expansion_factor` and `link_decay_factor` parameters to key A separate cache entries for different link settings
