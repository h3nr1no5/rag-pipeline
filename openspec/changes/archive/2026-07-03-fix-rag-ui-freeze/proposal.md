...
1. **Background task queue**: New `POST /query/start`, `GET /query/status/{id}` endpoints. Queries run as background `asyncio.Task`, UI polls for status.
...
### New Capabilities
- `async-query-execution`: Background task creation for RAG queries, status polling endpoint, and frontend polling loop that keeps the UI responsive
...
| Area | Impact |
|------|--------|
| `src/api/routes/query/routes.py` | Add 2 new endpoints (start, status) |
