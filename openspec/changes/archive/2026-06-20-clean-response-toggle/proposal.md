...
- **New `clean_response` field** on `QueryRequest` schema (boolean, default `True` for API backward compatibility)
...
- **Frontend sidebar**: Add "Clean Response" checkbox in Chat.py alongside existing "Show Citations", **defaulting to `False`** so new users see raw LLM output
- **Parameter persistence**: Save/load `clean_response` via `chat_params.json` alongside temperature, max_tokens, etc. — silently picked up on upgrade
- **Split defaults**: Backend API defaults to `True` (programmatic clients unaffected), frontend defaults to `False` (new UX)
...
- **Frontend**: `client/pages/3_💬_Chat.py` — new checkbox in sidebar (default `False`), add to `save_params()`/`load_saved_params()`
