...
- [ ] 1.1 Create `src/domain/services/task_manager.py` with `TaskRecord` and `BackendResult` dataclasses and `TaskManager` class
...
- [ ] 3.3 Refactor API Docs query into a callable function from the background task
...
- [ ] 3.4 Update TaskRecord schema to use `results: list[BackendResult]` instead of `result: dict | None`
...
- [ ] 4.1 Add `async_query_start()` and `async_query_poll()` helper functions
...
- [ ] 4.2 Create `@st.fragment`-decorated polling function
...
- [ ] 4.3 Replace ThreadPoolExecutor + as_completed() block (lines 534-616) with fragment-based flow
...
- [ ] 4.4 Add per-backend progressive rendering to fragment
...
- [ ] 4.5 Add document list caching during polling
...
- [ ] 4.6 Add network error handling with retry
...
- [ ] 4.7 Add loading state on initial submit
...
- [ ] 4.8 Add `models_ready` interaction handling during polling
...
- [ ] 4.9 Disable inputs while task is active
...
- [ ] 8.0 Verify `pyproject.toml` specifies Streamlit >= 1.33 (required for `st.fragment`)
...
- [ ] 8.5 Manual smoke test: submit a query, verify:
