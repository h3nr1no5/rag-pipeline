...
async def test_query_dspy_maps_output_correctly():
    ...
    with patch(
        ...
        mock_module.return_value = forward_result
    ...
    # module() called with the correct arguments (temperature=None since
    # _query_dspy does not pass a temperature override in the test)
    mock_module.assert_called_once_with(question="test query", top_k=5, temperature=None, max_tokens=None)  # noqa: E501
...
async def test_circuit_breaker_falls_back_when_dspy_fails():
    ...
    with patch(
        ...
        mock_instance.side_effect = RuntimeError("DSPy failed")
