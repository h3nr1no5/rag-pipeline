...
def _assert_realistic_answer(result: dict, expected_keyword: str = "material") -> None:
    """Assert answer is from RealisticTestLLM (not TestLLM) and contains document content."""
    test_llm_canned = "This is a test response generated by the test LLM for testing purposes."
    assert result["answer"] != test_llm_canned, (
        f"Got canned TestLLM response: {result['answer'][:100]}"
    )
...
def _realistic_llm():
    """Reseed LLM singleton with RealisticTestLLM for every test in this module.

    The session-scoped ``seed_singletons`` fixture (in ``tests/integration/conftest.py``)
    sets ``_llm_instance`` to ``TestLLM`` before any test runs.  This function-scoped
    autouse fixture runs after that session fixture and overrides it with
    ``RealisticTestLLM``, which parses ``[Source N]:`` sections from the prompt and
    returns a response containing actual document content — verifying that the full
    retrieval → prompt assembly → LLM pipeline is functional.
    """
...
async def test_cosine_pipeline(auth_client):
    ...
    _assert_realistic_answer(result)
