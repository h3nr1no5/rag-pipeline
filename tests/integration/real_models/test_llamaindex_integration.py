"""Real-model integration test for the LlamaIndex pipeline with profiling.

Uploads a real PDF with semantic chunking, exercises
``LlamaIndexRetriever.generate()`` with real ML models, asserts on
``[PROFILE]`` log output to confirm all six StepTimer probes fire,
...
async def auth_client(setup_test_db):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        test_email = f"li_profile_{uuid.uuid4().hex[:8]}@example.com"
        await ac.post(
            "/api/v1/auth/signup",
...
@pytest.mark.slow
@pytest.mark.asyncio
async def test_llamaindex_profile_lines(auth_client, caplog):
    """Upload ``test_pdf.pdf`` (semantic chunking), query the LlamaIndex
    pipeline, verify all six ``[PROFILE]`` log lines are emitted, and
    assert the answer contains ``"material"``.

    Expected probe names
    --------------------
    ``retrieve``, ``ensure_components``, ``hybrid_search``, ``rerank``,
